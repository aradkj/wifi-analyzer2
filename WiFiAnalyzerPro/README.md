...
## ۲. ویژگی‌ها و امکانات کلیدی

### عدم باز شدن پنجره‌های کنسول و پاورشل (کاملاً بی‌صدا و نامرئی)
* **هنگام اجرای برنامه:** به لطف پیکربندی `console=False` در فایل `WiFiAnalyzerPro.spec`، فایل اجرایی `WiFiAnalyzerPro.exe` صرفاً پنجره زیبای گرافیکی را باز می‌کند و هیچ پنجره کنسول، CMD یا PowerShell ظاهر نمی‌شود.
* **هنگام اسکن شبکه‌ها:** دستور `netsh` با بهره‌گیری از فلگ‌های اختصاصی ویندوز `subprocess.CREATE_NO_WINDOW` و تنظیم `STARTUPINFO` با وضعیت `SW_HIDE` در پس‌زمینه اجرا می‌شود. اجرای اسکن کاملاً بی‌صدا است و هیچ فلش، پرش یا پنجره مزاحمی روی مانیتور کاربر باز نمی‌شود.
...
### نقشه حرارتی کانال‌ها و نمودار زنگوله‌ای طیفی (Channel Heatmap)
* تفکیک باندهای فرکانسی **2.4 GHz** (کانال‌های ۱ تا ۱۴) و **5 GHz** (کانال‌های ۳۶ تا ۱۶۵).
* رسم منحنی‌های زنگوله‌ای بیزی (Bell Curves) متناسب با پهنای باند و هم‌پوشانی کانال‌ها (Channel Overlap).
* محاسبه هوشمند تداخل و رتبه‌بندی کانال‌های ۱، ۶ و ۱۱ و ارائه پیشنهاد بهترین کانال با کمترین تداخل.
